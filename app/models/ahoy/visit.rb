...
module Ahoy
  class Visit < ApplicationRecord
    ...
    VIEW_ANALYTICS_OPTIONS = ['Today', 'Yesterday', 'Last 7 Days', 'Last 28 Days', 'Last 90 Days'].freeze
    ...
    def self.filter_analytic_session(filter = {})
      ...
      datas.sort.reverse.each do |key, value|
        ...
        results << data if value.positive?
      end
    ...
    def self.get_visit_datas(filter = {})
      ...
      if view.present?
        start_date = case view
                     ...
                     when 'Last 28 Days'
                       (Date.today - 28.days).beginning_of_day
                     when 'Last 90 Days'
                       (Date.today - 90.days).beginning_of_day
                     end

        end_date   = view.eql?('Yesterday') ? start_date.end_of_day : Date.today.end_of_day
      end

      Ahoy::Visit.where('started_at BETWEEN ? AND ?', start_date, end_date)
    end
